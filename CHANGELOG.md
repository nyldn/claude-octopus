...
- **Gemini dispatch no longer defeats its own `~/.gemini/.env` fallback, and `NODE_EXTRA_CA_CERTS` survives env isolation** (#660). `build_provider_env()`'s Gemini branch presets `GEMINI_API_KEY`/`GOOGLE_API_KEY` as empty-but-set when the parent shell doesn't export them, which blocks gemini-cli's own dotenv loader (it won't override an already-set variable) and made every dispatch fail with "you must specify the GEMINI_API_KEY environment variable" even though a plain `gemini -p` from the same shell worked. Credentials and endpoint settings (`GOOGLE_CLOUD_PROJECT`, `GOOGLE_CLOUD_PROJECT_ID`, `GOOGLE_GEMINI_BASE_URL`, `NODE_EXTRA_CA_CERTS`) are now only forwarded into the isolated environment when actually set, and `NODE_EXTRA_CA_CERTS`/`GOOGLE_GEMINI_BASE_URL` are newly allowlisted so relay/custom-endpoint setups survive isolation.
