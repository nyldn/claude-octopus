...
# Coordinates multiple AI agents (Codex CLI, Antigravity CLI, Claude, etc.) for parallel task execution
...
# which sandboxed every provider (codex workdir, agy workspace) to the plugin
...
# Wraps codex/agy output in trust markers; passes claude output unchanged
# ═══════════════════════════════════════════════════════════════════════════════
...
ERROR_CODES=(
    ...
    "E002:Antigravity not signed in — run 'agy' once to complete interactive sign-in:agy:help api-setup"
    "E003:Codex CLI not found:npm install -g @openai/codex:help setup"
    "E004:Antigravity CLI not found:curl -fsSL https://antigravity.google/cli/install.sh | bash:help setup"
    ...
    "E009:Invalid agent type:Use codex, codex-mini, agy, claude:help agents"
...
is_agent_available() {
    ...
    # (e.g. perplexity 401) so it is not re-dispatched into the same
