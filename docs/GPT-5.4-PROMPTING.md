...
- **`/octo:review`** — dispatches to GPT-6 Sol by default (edge-case strength). Use `effort=medium` + output contract requiring `severity`, `file:line`, `rationale`. This document remains the compatibility guide for explicit GPT-5.4 pins.
