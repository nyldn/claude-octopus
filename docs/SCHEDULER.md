...
| **Process group isolation** | `setsid` puts orchestrate.sh and all its children (Codex CLI, Antigravity CLI, etc.) in a separate process group. `kill -- -$PGID` terminates the entire tree. |
...
| Antigravity CLI | Included with access/subscription | Antigravity |
...
- Missing credentials for configured metered providers (for example, `OPENAI_API_KEY`)
