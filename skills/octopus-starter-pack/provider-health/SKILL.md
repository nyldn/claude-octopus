...
2. **Summarize per provider.** For each seat report: available or not, auth method (env key, oauth session, subscription), and the model it would resolve to today. Include the claude-sdk seat when `CLAUDE_SDK_API_KEY` is set (Agent SDK, Opus 5 by default, 1M context). Flag Claude Code older than v2.1.280 for Opus 5.5 or older than v2.1.219 for Opus 5. For GPT-6, explain that a current Codex client does not prove account or workspace rollout access; retain GPT-5.6 Sol as the compatibility fallback.
