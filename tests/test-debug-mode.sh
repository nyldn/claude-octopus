#!/usr/bin/env bash
# Test suite for debug mode (v7.25.0)
...
output=$("$ORCHESTRATE" --debug probe "test debug mode" --dry-run 2>&1)
...
output=$(OCTOPUS_DEBUG=true "$ORCHESTRATE" probe "test debug env" --dry-run 2>&1)
...
output=$("$ORCHESTRATE" probe "test no debug" --dry-run 2>&1)
assert_not_contains "$output" "DEBUG" "No debug output without --debug"

# Test 4: Debug shows model selection
echo ""
echo "Test 4: Debug shows model selection"
output=$("$ORCHESTRATE" --debug probe "test" --dry-run 2>&1)
assert_contains "$output" "Model from\|Using hard-coded default" "Shows model selection source"
...
# Test 6: Debug shows probe_discover details
echo ""
echo "Test 6: Debug shows probe_discover details"
output=$("$ORCHESTRATE" --debug probe "test" --dry-run 2>&1)
assert_contains "$output" "probe_discover:" "Shows probe_discover debug info"

# Test 7: --debug sets VERBOSE=true automatically
echo ""
echo "Test 7: --debug enables verbose mode"
output=$("$ORCHESTRATE" --debug probe "test" --dry-run 2>&1)
assert_contains "$output" "INFO" "Verbose mode enabled with debug"

# Test 8: Help shows --debug option
echo ""
echo "Test 8: Help text includes --debug"
