...
estimate_workflow_cost() {
    ...
    # agy cost
    if is_api_based_provider "agy"; then
        local per_call
        per_call=$(calculate_agent_cost "agy" "$prompt_length")
        agy_cost=$(awk "BEGIN {printf \"%.2f\", $per_call * $agy_calls}")
        local agy_high
        agy_high=$(awk "BEGIN {printf \"%.2f\", $agy_cost * 1.5}")
        agy_label="~\$${agy_cost}-${agy_high} (${agy_calls} calls, API key)"
        has_any_cost=true
    else
        agy_label="Included (auth-connected)"
    fi
...
show_cost_estimate() {
    ...
    echo -e "    ${YELLOW}🟡${NC} agy:    ${agy_label}"
...
display_workflow_cost_estimate() {
    ...
    local agy_status="Subscription (no per-call cost)"
    ...
    if [[ "$agy_is_api" == "true" ]]; then
        agy_cost=$(awk "BEGIN {printf \"%.2f\", $(calculate_agent_cost \"agy\" \"$prompt_size\") * $num_agy_calls}")
        agy_status="~\$$agy_cost (API key detected)"
    fi
    ...
    local total_cost=$(awk "BEGIN {printf \"%.2f\", $codex_cost + $agy_cost + $perplexity_cost}")
    ...
    echo -e "  ${YELLOW}🟡 agy${NC}    (~${num_agy_calls} requests): ${agy_status}"
    ...
    if [[ $(awk "BEGIN {print ($total_cost > 0)}") -eq 1 ]]; then
        ...
        echo ""
        echo -e "${DIM}Note: Costs shown only for providers using API keys (OPENAI_API_KEY/GOOGLE_API_KEY/PERPLEXITY_API_KEY).${NC}"
