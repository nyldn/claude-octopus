...
- **Copilot dispatch failed validation and aborted the whole phase, killing sibling in-flight agents.** `validate_agent_command()` never allowlisted the `copilot-exec.sh` shim path returned by `dispatch.sh` since v9.9.0 — only the bare `copilot` command. Review also caught that the fix's original suffix-match pattern (and the pre-existing, identically-shaped `agy-exec.sh` arm) accepted any command *ending* in the shim path regardless of its actual executable; both are now validated against the extracted executable token instead, with regression tests for the accept and embedded-exploit paths. (#697, #705, #702)
