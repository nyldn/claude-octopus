...
DIRECTIVE='🐙 FABLE 5 MODE ACTIVE — a claude-fable-5 pin was detected. Three guards are auto-enforced by orchestrate.sh (OCTOPUS_FABLE5_MODE=off to disable): (1) security-audit dispatches reroute to Opus 5 by default because Fable 5 safety classifiers can refuse adversarial security phrasing; OCTOPUS_FABLE5_FALLBACK_MODEL may replace that target; (2) effort clamps xhigh/max to high — Fable 5 effort applies per tool call and higher settings widen scope at 2x cost without extending runs; (3) the claude-sdk seat retries a refused/empty Fable 5 dispatch once on the same configured fallback. When writing prompts for Fable 5 dispatches: never ask it to reveal or transcribe its reasoning (triggers the reasoning_extraction refusal); no token or context countdowns; drop CRITICAL/MUST emphasis unless strict compliance is required; prefer a boundary plus checkable acceptance criteria over micromanaged step plans. Full profile: skills/blocks/fable5-prompting.md.'
