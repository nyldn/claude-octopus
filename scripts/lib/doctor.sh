...
doctor_check_providers() {
    ...
    if [[ -f "$fallback_log" ]]; then
        local recent_failures=0 codex_failures=0
        local cutoff
        ...
        if [[ $recent_failures -gt 0 ]]; then
            ...
            doctor_add "provider-fallbacks" "providers" "warn" \
                ...
                "${detail}. Check auth: codex auth. Log: ${fallback_log}"
        else
...
doctor_check_auth() {
    ...
    if [[ "$any_auth" == "false" ]]; then
        doctor_add "any-provider-auth" "auth" "fail" \
            "No provider authenticated" "At least one of Codex or Cursor Agent must be authenticated"
    else
