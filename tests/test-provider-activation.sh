...
# 4.1: resolve_octopus_model uses the GPT-6 Sol current-model picker
if grep -rA 3 '^codex_default_model()' $SCRIPTS_ALL | grep -q 'gpt-6-sol' &&
   grep -rA 10 "case \"\$agent_type\" in" $SCRIPTS_ALL | grep -A 1 "codex\*)" | grep -q 'codex_default_model'; then
    pass "4.1 resolve_octopus_model returns gpt-6-sol for codex default"
else
