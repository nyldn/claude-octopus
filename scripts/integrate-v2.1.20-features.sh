...
check_providers_with_mcp() {
    if [[ -f "$MCP_DETECTION" ]]; then
        ...
        echo '{"detection_method":"cli","providers":{"codex":{"status":"unknown"},"agy":{"status":"unknown"},"claude":{"status":"available"}}}'
    fi
}
...
request_background_permission_if_needed() {
    ...
    if [[ -f "$PERMISSIONS_MANAGER" ]]; then
        if ! "$PERMISSIONS_MANAGER" check "$workflow" "$autonomy" "codex agy"; then
            return 1
        fi
    fi
