...
**Fork merge: upstream v8.12.0 + fork fixes**
- Merged all upstream features from v8.8.0 through v8.12.0 into fork
- Resolved 8 file conflicts (namespace, version, orchestrate.sh delivery patterns)
- Adopted upstream Gemini headless mode improvements (Issue #25)
- Adopted upstream stdin-based prompt delivery pattern for Gemini
- Preserved fork-specific fixes: simplified skills, bash strict mode, provider invocation embedding
...
- **Namespace rename**: `nyldn` -> `adrivellen` across package.json, README.md, marketplace.json, test-plugin-lifecycle.sh
