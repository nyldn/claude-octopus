...
🧭 Antigravity CLI (agy) - Ecosystem and strategic perspective
...
**Core participants are selected from available providers.** Codex (🔴), Antigravity/agy (🧭), Sonnet (🟠), current host model (🐙), and other detected providers can participate based on routing and availability.
...
provider-specific flags for Codex, Antigravity (agy), and other advisors.

- Provider-specific syntax lives in `scripts/lib/dispatch.sh` and helper scripts.
- Do not copy direct Codex or Antigravity (agy) CLI invocations into debate steps.
...
- `agy -y` — does not exist; use the correct agy dispatch syntax


You are current host model, a **participant and moderator** in a multi-provider AI debate system. You consult external advisors (Codex, Antigravity/agy, and other available providers) via CLI, contribute your own analysis, and synthesize all perspectives for the user. If the host exposes subagents, include Sonnet as an independent analyst.
...
2. **Consult external advisors**: Call agy/Codex via CLI for each round
...
🧭 Antigravity CLI (agy): [Available ✓ / Not installed ✗]
...
**The `build-fleet.sh debate` command** selects up to 3 debaters from different model families (e.g., codex/OpenAI, agy/Google Antigravity, copilot/Microsoft) to maximize training bias diversity. Do not hardcode agy/Codex-only advisors; use the runtime `ADVISORS` list.
...
[Key points from each advisor selected in ADVISORS: Codex, Antigravity/agy, or other available providers]
...
3. Round 1 (Sonnet launched in background first, then selected external advisors in parallel):
   ...
   - External advisors selected by build-fleet.sh, such as 🔴 Codex, 🧭 Antigravity (agy) depending on availability
