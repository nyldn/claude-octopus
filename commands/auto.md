...
### Research Intent
```bash
/octo:auto research OAuth security patterns
...
### Build Intent (Clear)
```bash
/octo:auto build user authentication with JWT
...
### Build Intent (Vague)
```bash
/octo:auto build something for users
...
### Validation Intent
```bash
/octo:auto validate the authentication implementation
...
### Debate Intent
```bash
/octo:auto should we use TypeScript or JavaScript?
...
### Lifecycle Intent
```bash
/octo:auto complete implementation of payment system
...
/octo:auto research advanced ML architectures
...
/octo:auto build and validate authentication system
...
When the user invokes `/octo:auto <query>` or says `octo <query>`, you MUST:
...
```
User: /octo:auto research OAuth 2.0 security best practices
...
```
User: /octo:auto create authentication
...
```
User: /octo:auto do something with the database
...
```
User: /octo:auto should we use Redis or Memcached for session storage?
