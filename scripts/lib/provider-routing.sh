...
build_provider_env() {
    ...
    # external CLIs (codex/perplexity) participate in distributed traces.
        ...
        cat > "$tmp_file" << EOF
{
  ...
  "tiers": {
    "budget": { "codex": "mini" },
    "standard": { "codex": "default" },
    "premium": { "codex": "default" }
  },
  "overrides": {}
}
EOF
...
set_provider_model() {
    ...
    case "$provider" in
        codex|claude|perplexity|opencode|openrouter|openai-compatible-agent|cursor-agent) ;;
        *)
            if [[ "${4:-}" != "--force" ]]; then
                echo "ERROR: Unknown provider '$provider'. Valid: codex, claude, perplexity, opencode, openrouter, openai-compatible-agent, cursor-agent" >&2
...
reset_provider_model() {
    ...
    if [[ "$provider" == "all" ]]; then
    ...
    elif [[ "$provider" =~ ^(codex|claude|perplexity|opencode|openrouter|openai-compatible-agent|cursor-agent)$ ]]; then
        ...
        echo "ERROR: Invalid provider '$provider'. Use 'codex', 'claude', 'perplexity', 'opencode', 'openrouter', 'cursor-agent', or 'all'" >&2
        return 1
    fi
...
get_alternate_provider() {
    ...
    case "$locked_provider" in
        codex|codex-fast|codex-mini)
            if ! is_provider_locked "claude-sonnet"; then
                echo "claude-sonnet"
            else
