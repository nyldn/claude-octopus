...
# Gemini-CLI-family creds (qwen) store `expiry_date` as epoch MILLISECONDS.
...
handle_auth_command() {
    ...
    case "$action" in
        ...
        status)
            ...
            # Check Antigravity (agy) — Google seat (Gemini CLI sunset 2026-06-18)
            echo ""
            if command -v agy &>/dev/null; then
                echo -e "  Antigravity: ${GREEN}✓ CLI installed${NC}"
                echo "               Run 'agy' once to complete interactive sign-in if needed"
            else
