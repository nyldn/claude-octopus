...
if echo "$COMMAND" | grep -qE '(ufw\s+allow|iptables.*ACCEPT|--publish|security-list.*18789).*18789'; then
    echo '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"Sysadmin safety gate: exposing OpenClaw gateway port 18789 to public internet is unsafe — use Tailscale or VPN instead"}}'
    exit 0
fi

# 3. docker compose down -v (destroys volumes)
if echo "$COMMAND" | grep -qE 'docker\s+compose\s+down\s+-v'; then
    echo '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"Sysadmin safety gate: docker compose down -v destroys all volumes and data — remove -v flag or backup first"}}'
    exit 0
fi
...
if echo "$COMMAND" | grep -qE '(ufw\s+(disable|reset)|pfctl\s+-d|systemctl\s+(stop|mask)\s+(ufw|firewalld|nftables))'; then
    echo '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"Sysadmin safety gate: disabling firewall entirely is unsafe — modify rules instead"}}'
    exit 0
fi

# 5. Dropping all iptables rules
if echo "$COMMAND" | grep -qE 'iptables\s+-F'; then
    echo '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"Sysadmin safety gate: flushing all iptables rules removes firewall protection"}}'
    exit 0
fi
...
if echo "$COMMAND" | grep -qE '(curl|wget).*\|\s*sudo\s+(ba|z)?sh'; then
    echo '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"Sysadmin safety gate: piping downloads to sudo sh is dangerous — download and inspect the script first"}}'
    exit 0
fi
if echo "$COMMAND" | grep -qE 'sudo\s+(ba|z)?sh\s+-c\s+.*\$\((curl|wget)'; then
    echo '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"Sysadmin safety gate: running downloaded content as root via command substitution is dangerous"}}'
    exit 0
fi

# 7. Destroying Proxmox VMs/containers without confirmation keyword
if echo "$COMMAND" | grep -qE '(qm|pct)\s+destroy'; then
    echo '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"Sysadmin safety gate: VM/container destruction requires explicit user confirmation"}}'
    exit 0
fi
