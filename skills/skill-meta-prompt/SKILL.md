...
- **Current frontier roster**: apply `skills/blocks/frontier-model-routing.md` when choosing between Opus 5, GPT-6, GPT-5.6 fallbacks, Sonnet 5, Fable 5.1, Astra, or cheaper seats.
...
- **GPT-6 Astra**: reserve it for a bounded pass after GPT-6 Sol fails a hard acceptance test. Do not treat two OpenAI models as independent review.
- **Codex / GPT-6 and GPT-5.6**: apply `docs/GPT-5.6-PROMPTING.md`; give it a concrete outcome, scoped repository constraints, checkable acceptance criteria, non-goals, and the required verification.
