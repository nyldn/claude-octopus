...
- **Council model selection: agy pin stdin leak and org-diversity short-circuit** (#600, reported and root-caused by @dustinspace217 in #599): `validate_agy_model_name()` ran `agy models` with stdin inherited from the dispatch pipeline, so a piped prompt was read by `agy` as chat input instead of a model list — a valid `OCTOPUS_AGY_MODEL` pin was rejected and the seat silently downgraded to the default model. `agy models` is now called with stdin redirected from `/dev/null`. Separately, `council_pick_provider()` short-circuited to the preferred provider whenever it was merely available, without checking whether that provider's org was already seated on the roster, so the org-diversity fallback only ran when the preferred provider was down — with claude/codex/agy all healthy, an available `opencode` seat was never used. The short-circuit is now gated on the org not already being represented, matching the diversity loop's own check. A third issue reported in #599 (mismatched provider/model labels in `summary.json`) is a design call the reporter left open and is not addressed here.
