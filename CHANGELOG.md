...
- **SessionEnd hooks and event logging no longer fail on an unwritable HOME** (#648). A Claude process nested under a restricted sandbox (e.g. a child of a Codex `workspace-write` sandbox) can inherit a `CLAUDE_PLUGIN_DATA`/`HOME` path that exists but rejects writes, which aborted `session-end.sh`/`workflow-verification.sh` under `set -e` and leaked a bare "Operation not permitted" from `scripts/lib/events.sh`. The leak was a redirection-ordering bug — `>> file 2>/dev/null` still prints bash's own error when `>>` itself is what fails, since the stderr suppression hadn't taken effect yet; the fix orders `2>/dev/null` first everywhere this pattern appears. `orchestrate.sh` now probes the resolved workspace root for writability and falls back to a run-scoped `$TMPDIR` directory, and every mkdir/write/mv in `session-end.sh`'s best-effort bookkeeping (auto-memory preferences, session learnings) is now guarded so a permission failure degrades to a no-op instead of failing the hook.
