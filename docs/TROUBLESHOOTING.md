...
**Fable 5 dispatch refused or empty** — expected for security-audit phrasing on `claude-fable-5` pins; the plugin reroutes security passes to Opus 5 and retries refused claude-sdk dispatches automatically. Details: `skills/blocks/fable5-prompting.md`. Disable the guards with `OCTOPUS_FABLE5_MODE=off`.
