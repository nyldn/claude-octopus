...
# Copies workflow state and provider env vars into the worktree for continuity
...
# Copy .octo state into worktree for workflow state continuity
if [[ -d ".octo" && -d "$WORKTREE_PATH" ]]; then
...
# Write .octopus-env with provider API keys for env isolation
