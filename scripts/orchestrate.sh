...
run_with_timeout() {
    ...
    if command -v gtimeout &>/dev/null; then
        ...
        # Explicit <&0 so background shell-function jobs inherit stdin from the pipe.
        # Non-interactive bash redirects stdin to /dev/null for `cmd &` without this.
        "$@" <&0 &
        cmd_pid=$!
...
EOF
    ...
    # Extract content from response (nested path: .choices[0].message.content)
    local content=""
    if command -v jq &>/dev/null; then
